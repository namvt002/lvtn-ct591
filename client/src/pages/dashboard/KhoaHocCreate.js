...
import KhoaHocNewForm from "../../components/_dashboard/khoahoc/KhoaHocNewForm";

// ----------------------------------------------------------------------

export default function KhoaHocCreate() {
    ...
    useEffect(() => {
        (async () => {
            if (isEdit) {
                const _res = await getData(API_BASE_URL + `/khoahoc/${id}`);
    ...
    return (
        <Page title="KhoaHoc | Learn Code">
            <Container maxWidth={themeStretch ? false : 'lg'}>
                ...
                />
                <KhoaHocNewForm isEdit={isEdit} currentProduct={current} id={id}/>
            </Container>
        </Page>
    );
}
